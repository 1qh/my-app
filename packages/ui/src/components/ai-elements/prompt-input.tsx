...
import { InputGroup, InputGroupAddon, InputGroupButton, InputGroupTextarea } from '@workspace/components/ui/input-group'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@workspace/components/ui/select'
...
export const PromptInputProvider = ({ children, initialInput: initialTextInput = '' }: PromptInputProviderProps) => {
  ...
  const [textInput, setTextInput] = useState(initialTextInput),
    ...
    [attachements, setAttachements] = useState<(FileUIPart & { id: string })[]>([]),
  ...
  return (
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            'group relative flex h-8 cursor-default items-center gap-1.5 rounded-md border border-border px-1.5 text-sm font-medium transition-all select-none hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50',
            className
          )}
          key={data.id}
          {...props}>
...
export const PromptInput = ({
  ...
  const controller = useOptionalPromptInputController(),
    ...
    anchorRef = useRef<HTMLSpanElement>(null),
  ...
  const [items, setItems] = useState<(FileUIPart & { id: string })[]>([]),
    ...
    openFileDialogLocal = useCallback(() => {
    ...
    add = usingProvider ? (files: File[] | FileList) => controller.attachments.add(files) : addLocal,
    remove = usingProvider
  ...
  useEffect(
    () => () => {
      if (!usingProvider) for (const f of files) if (f.url) URL.revokeObjectURL(f.url)
    },
    [usingProvider, files]
  )
  ...
  const handleChange: ChangeEventHandler<HTMLInputElement> = event => {
    ...
    convertBlobUrlToDataUrl = async (url: string): Promise<string> => {
